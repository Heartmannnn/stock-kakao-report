...
#   2. PRIORITIZES STOCK/INVESTMENT TALK over everyday chatter (하이닉스, 샌디스크, 매입, 매도, 보유 유추)
#   3. Normalizes participant language traits (e.g., '안재웅' replaces '이/e' with '2': 하2닉스 -> 하이닉스, 네2버 -> 네이버)
#   4. Deduces holdings from sentiment/complaints (e.g., "하닉 떨어졌어 ㅠㅠ" -> Holds SK Hynix)
#   5. Lists unknown terms/slang for user clarification
#   6. Dynamically extracts ALL unique participants on that day
#   7. Appends source chat log filename at the end of report & KakaoTalk message
#   8. Cleans up older chat log files automatically
# ==============================================================================
...
# Domain Investment Keywords for prioritized parsing & fallback
INVESTMENT_KEYWORDS = [
    "주식", "주가", "반도체", "유가", "환율", "달러", "금", "금시세", "금 시세", "원달러",
    "뇌동매매", "존버", "떡상", "떡락", "시드", "구조대", "가즈아", "물타기", "손절", "익절", "풀매수", "야수", "돔황챠",
    "하이닉스", "하닉", "하2닉스", "샌디스크", "엔비디아", "NVDA", "마이크로소프트", "MSFT", "애플", "AAPL", "테슬라", "TSLA", "아람코", "삼성전자", "SK하이닉스", "스페이스X", "QQQ", "SPY", "VOO",
    "PER", "PBR", "CapEx", "실적발표", "금리", "FOMC", "CPI", "PPI", "고용보고서", "해외ETF", "ISA", "비과세", "투자", "매수", "매도", "추매", "관망", "매입", "샀다", "팔았다", "망했네", "떨어졌어"
]
...
def normalize_participant_message(name, msg):
    norm = msg
    # Language Trait Rule: '안재웅' replaces '이' or 'e' with '2' (e.g. 하2닉스 -> 하이닉스, 네2버 -> 네이버, 2더 -> 이더)
    if "안재웅" in name:
        norm = norm.replace("하2닉스", "하이닉스").replace("2더리움", "이더리움").replace("2더", "이더").replace("네2버", "네이버").replace("2지스", "이지스")
...
PRIORITY INVESTMENT PARSING RULES:
1. PRIORITIZE STOCKS OVER CASUAL TALK: Ignore everyday chatter (games, food) if stock/investment/oil/FX/gold talk is present.
2. TRACK BUYING/SELLING: If any participant mentions buying ("매입", "샀다", "진입") or selling ("매도", "팔았다", "익절", "손절"), NEVER miss it. Record their exact transaction in Section 1.
3. DEDUCE HOLDINGS FROM SENTIMENT: If someone complains or mentions price movements of a stock (e.g. "아 하닉/하이닉스 떨어졌어 ㅠㅠ", "샌디스크 망했네 ㅠㅠ"), DEDUCE that they HOLD that stock and infer their portfolio stance!
4. LANGUAGE TRAITS: Participant '안재웅' replaces '이' or 'e' with '2' (e.g. '하2닉스' = 하이닉스, '2더' = 이더리움, '네2버' = 네이버). Correctly map his stock mentions.
5. UNKNOWN TERM ESCALATION: If there are obscure slang or non-obvious abbreviations, list them at the end under '❓ [수석 애널리스트 용어/비속어 확인 요청]'.
...
4. ESTIMATED PORTFOLIO: Section 2 MUST estimate current stock/asset holdings and percentages based on deducted chat topics.
...
{chat_text}"""
    ...
    # Dynamic Fallback Report scanning for investment keywords & holdings per participant
    table_rows = []
    if participants_map:
        for name, msgs in participants_map.items():
            ...
            # Deduce holdings or stock ticker from message
            ticker = "S&P500 / 주요 자산"
            pos = "관망 / 공유"
            if "하이닉스" in selected_msg or "하닉" in selected_msg or "하2닉스" in selected_msg:
                ticker = "SK하이닉스"
                pos = "보유 / 추적" if ("떨어졌" in selected_msg or "망했" in selected_msg) else "관망/탐색"
            elif "샌디스크" in selected_msg:
                ticker = "샌디스크 (메모리)"
                pos = "보유 (손실)" if ("망했" in selected_msg or "떨어졌" in selected_msg) else "보유/관망"
            elif "스페이스X" in selected_msg or "스페이스x" in selected_msg:
                ticker = "스페이스X / 비상장"
                pos = "매수 탐색"
            ...
            table_rows.append(f"| **{name}** | {ticker} | **{pos}** | {selected_msg} |")
    else:
        table_rows.append("| **주요 참여자** | S&P500 / SK하이닉스 | **관망 / 적립** | SK하이닉스, 샌디스크, 환율 및 주식 시황 공유 |")
...
- **팩트체크:** 오늘 참여자({participants_str})들 주식(하이닉스/샌디스크), 반도체, 환율 시황 체크하면서 뇌동매매 안 하고 차분하게 잘 대응했네!
...
def format_kakao_message(report_text, report_url, today_str, participants_map, chat_filename):
    ...
    msg_lines = [
        ...
        "",
        "💡 [수석 애널리스트 솔직 훈수]",
        "• 팩트체크: 주식/하이닉스/환율 시황 체크하며 차분히 잘 반응함!",
...
def main():
    ...
    # 2. Filter chat strictly starting from LAST DATE HEADER & normalize language traits (e.g. 안재웅)
    chat_text, participants_map, date_label = read_and_filter_today_chat(chat_file)

    # 3. Generate Chart PNG
    repo_root = os.path.dirname(os.path.abspath(__file__))
    if os.path.basename(repo_root) == "project3_chat_analyzer":
        repo_root = os.path.dirname(repo_root)
    chart_path = os.path.join(repo_root, "portfolio_chart.png")
    generate_portfolio_chart(chart_path)

    # 4. Generate Soul Company Markdown Report with specific trading/holdings deduction rules
