...
def get_latest_report():
    ...
    files = [f for f in glob.glob(pattern) if "report" in os.path.basename(f) and os.path.basename(f) != "kakao_setup_guide.md"]
    if not files:
...
def format_report_for_kakao(file_path):
    ...
    for c_line in cause_lines[:3]:
        ...
        if len(clean_cause) > 60:
            clean_cause = clean_cause[:57] + "..."
    ...
    lines_list.append("------------------------------------")
    lines_list.append("상세 내용: GitHub 저장소 내 markdown 리포트 참조")
...
def main():
    ...
    formatted_msg = format_report_for_kakao(report_file)
    ...
    try:
        result = send_kakao_memo(access_token, formatted_msg)
        if result.get("result_code") == 0:
            print("\n🎉 [성공] 카카오톡 나와의 채팅방으로 주식 리포트가 성공적으로 발송되었습니다!")
        else:
