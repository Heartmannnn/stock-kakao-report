...
if (-not $PSScriptRoot) { $CurrentDir = Get-Location }
...
# Automatically locate the latest KakaoTalk chat log file
$SearchPaths = @(
...
if ($FilteredLines.Count -gt 250) {
    $StartIndex = $FilteredLines.Count - 250
    $FilteredLines = [System.Collections.ArrayList]($FilteredLines.GetRange($StartIndex, 250))
}
...
# Format and Send KakaoTalk Memo
